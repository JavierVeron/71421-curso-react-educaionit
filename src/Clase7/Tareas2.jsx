...
const TasksReducer = (prevState = tasks, action) => {
    switch(action.type) {
        ...
        case "SHOW_ALL":            
            return tasks
        case "FILTER_TASKSBYNAME":
            return tasks.filter(item => item.nombre.toUpperCase().includes(action.payload.toUpperCase()))
        default:
            return prevState
    }
}
...
const Tareas2 = () => {
    ...
    const FILTER_TASKSBYNAME = (value) => (
        {type:"FILTER_TASKSBYNAME", payload:value}
    )
    ...
    const filtrarPorNombre = (name) => {
        setNameTask(name);
        store.dispatch(FILTER_TASKSBYNAME(name));
    }
        ...
        <div className="container my-5">
            <div className="row">
                <div className="col">
                    ...
                    <div className="my-3">
                        <input type="text" className="form-control" value={nameTask} onInput={(e) => {filtrarPorNombre(e.target.value)}}  />
                    </div>
